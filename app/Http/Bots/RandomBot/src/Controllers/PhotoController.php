...
class PhotoController extends BaseController
{
    ...
    {
        try {
            $post = PostRep::getRandomPhoto();
            //throw new Exception($post->file_id);
            if ($post->blocked->where('user_id', '=', $chat->chat_id)->isEmpty()) {
                $response = Http::post(sprintf($this->url . 'sendPhoto', $this->bot->token), [
                ...
                throw new Exception('Картинок больше нет(');
            }
